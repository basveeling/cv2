...
class PointChaining(object):
    def __init__(self, n_iterations, images):
        self.match_dist_threshold = 100000 # TODO: find a correct value for this
    ...
    def run_pipeline(self):
        ...
        for ind1 in range(n):
            ...
            # drawMatches(img1, kp1, img2, kp2, dmatches) # TODO: drawing of matches
            agreeing_matches = self.find_agreeing_matches(matches, dmatches, F)
    ...
    def find_agreeing_matches(self, matches, dmatches, F):
        homo_coords1, homo_coords2 = self.add_homogenous(matches[0]), self.add_homogenous(matches[1])
        n_matches = homo_coords1.shape[0]
        agreeing_matches = []
        for i in range(n_matches):
            if dmatches[i].distance < self.sift_distance_thres:
